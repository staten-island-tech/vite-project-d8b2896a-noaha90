...
function cardcreate(l){
  ...
  l.forEach(box => {
    document.querySelector(".container").insertAdjacentHTML(
     ...
     <img class="${box.border}" src="${box.image}" alt="${box.tagline}">
     <h2>${box.tagline}</h2>
 </div> `
  );
}
...
function filters(){
  ...
  let cat = filter.textContent.toLowerCase()
  console.log(cat)
  let newArr = array.filter((item) => item.type.includes(cat));
  console.log(newArr)
...
/* SLIGHTLY LESS AND ONLY SLIGHTLY LESS STUPID VERSION:
